...
Emitter.listen(
	'msg-login',
	(e: { msg: string; title: string; time?: number }) => {
...
<template>
	<header
		style="
			position: fixed;
			height: 100px;
			border-bottom: 2px solid #b19d95;
			z-index: 1;
		"
			...
			<nav style="margin-top: -10px; margin-right: 15px">
				...
				<ul v-if="!userLogged">
					...
					<nav v-else>
						...
						<li>
							<router-link to="/createUser">
								<ToolButton @click="createUserVisibleLoginFalse"
									>Criar conta</ToolButton
								>
							</router-link>
						</li>
					</nav>
				...
				<ul v-else>
					<nav>
						<li>
							<div
								@click="switchVisibleBtUserLogged"
								ref="btUserLogged"
	...
	async mounted() {
		...
		setTimeout(() => {
			try {
				...
				if (btEntrar !== null) Emitter.emitt('login', btEntrar);
			} catch {}
		}, 300);
