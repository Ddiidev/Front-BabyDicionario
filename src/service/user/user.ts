...
export async function dataCurrentUserLogged(): Promise<UserLogged> {
	...
	try {
		dataUserLogged.token = JSON.parse(
			localStorage.getItem('access_secure') ?? '',
		);
		const objSub = parseJwt<{ sub: string }>(
			dataUserLogged.token.access_token,
		);
		dataUserLogged.uuid = objSub.sub;

		try {
			const userDetails = await axios.get<IContractApi<UserLogged>>(
			...
			if (
				...
				dataUserLogged = Object.assign(
					dataUserLogged,
					userDetails.data.content!,
				);
		} catch (err) {
