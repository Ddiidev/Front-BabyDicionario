...
export async function mounted() {
	...
	userLoggedData.currentImage = await imageProfile(
		CurrentUserLogged.userLogged.uuid!,
		'default-user',
		getSexDefaultFromResponsible(CurrentUserLogged.userLogged.responsible!),
	);
}
		...
		const EventHandleResize = () => {
			...
			if (divFloat !== undefined && tagRef !== undefined) {
				...
				if (res.width < 530) {
					...
					divFloat.style.left =
						tagRefPos.left - divFloat.clientWidth + 35 + 'px';
				}
				self.$forceUpdate();
			}
		};
