...
<template>
	...
	>
		<Toast
			...
			:show="toast.show"
			@closeButton="toastData.removeMessage(toast)"
		></Toast>
		...
		<dialog v-if="modal.show" open>
			<article class="default-border">
				<header>
					...
					<p>
						<strong
							>📩 Aparentemente esse email já foi
							registrado.</strong
						>
					</p>
				</header>
		...
		<div class="grid">
				...
				<label>
					<fieldset v-if="current_step === 1">
						<label>
							...
							<input
								...
								v-model="form_data.primeiro_nome"
							/>
							<small
								>Dica: aqui é o nome do papai ou da mamãe. Logo
						...
						<label>
							<div v-if="form_data.primeiro_nome !== ''">
								{{ form_data.primeiro_nome }}, informe sua
								idade:
							</div>
							<input
								type="date"
								name="date"
								aria-label="Date"
								v-model="form_data.data_nascimento"
								aria-describedby="valid-data-nascimento"
					...
					<fieldset v-if="current_step === 2">
						<label>
							...
							<input
								v-model="form_data.senhaConfirm"
								name="pass_confirm"
					...
					<fieldset v-if="current_step === 3">
						<label>
							...
							<small
								>Será enviado um código de confirmação para seu
								e-mail</small
							>
					...
					<fieldset v-if="current_step === 4">
						<label>
							...
							<input
								v-model="form_data.codigoConfirmacao"
								placeholder="Digite o código enviado"
								aria-describedby="valid-code"
								:aria-invalid="!form_data.codigoValido"
								aria-label="Password"
							/>
							<small id="valid-code">
								{{
									form_data.codigoValido
										? 'Perfeito!!'
										: 'Código inválido'
								}}
							</small>
					...
					<ToolButton :onclick="prevStep" v-if="current_step > 1">
						&lt;&lt; Voltar</ToolButton
					>
					<ToolButton
						:onclick="codeVerificationMatch"
						v-if="current_step == 4"
						>Verificar código</ToolButton
					>
					<ToolButton v-if="current_step != 4" :onclick="nextStep"
						>Avançar >></ToolButton
					>
				</label>
			</article>
			<div id="body">
				<article
					v-if="ResponsiveUI.defaultMessageContainer"
