...
export async function mounted(self: any) {
	...
	data.profile.father!.currentImage = await imageProfile(CurrentUserLogged.userLogged.uuid!, data.profile.father?.uuid!, data.profile.father?.sex!);
	data.profile.mother!.currentImage = await imageProfile(CurrentUserLogged.userLogged.uuid!, data.profile.mother?.uuid!, data.profile.mother?.sex!);
...
export async function getImageCurrentProfile(profile: IProfile): Promise<string | undefined> {
	return await imageProfile(CurrentUserLogged.userLogged.uuid!, profile.uuid!, profile.sex!);
}

export function openSettingsProfile(profile: IProfile) {
	router.push(`/userProfileEdit/${profile.short_uuid}/${profile.name_shared_link}`)
}
