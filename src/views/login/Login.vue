            ...
            <form v-if="!passwordReset" name="body" @submit="handleSubmit">
                <fieldset>
                    ...
                    <label>
                        Senha
                        <fieldset role="group">
                            <input :type="visiblePass ? 'text' : 'password'" name="password" placeholder="Sua senha"
                                autocomplete="Password" v-model="password" :aria-invalid="invalidPassword" />
                        ...
                        <a class="secondary" @click="LoginImpl.beginPasswordReset"><small>Esqueceu a senha?</small></a>
                    </label>
                </fieldset>
            ...
            <form v-else name="passwordReset" @submit="handleSubmit">
                <fieldset v-if="flowPasswordReset == 1">
                    <label>
                        Email
                        <input ref="emailLogin" v-model="email" :aria-invalid="invalidEmail" type="email" name="email"
                            placeholder="Email" autocomplete="email" aria-describedby="valid-email" />

                        <small id="valid-email">{{ validEmailMessage }}</small>
                    </label>

                    <ToolButton style="width: 100%;" @click="LoginImpl.passwordResetNextStep">Começar</ToolButton>
                </fieldset>
                ...
                <fieldset v-if="flowPasswordReset == 2">
                    <label>
                        Nova senha
                        <fieldset role="group">
                            <input :type="visibleNewPassRecovery ? 'text' : 'password'" name="password"
                                placeholder="Uma nova senha" autocomplete="Password" v-model="newPasswordRecovery"
                                :aria-invalid="invalidNewPasswordRecovery" />
                            <button @click="visibleNewPassRecovery = !visibleNewPassRecovery" class="outline">
                                <EyeOpen v-if="visibleNewPassRecovery"></EyeOpen>
                                <EyeClose v-else></EyeClose>
                            </button>
                        </fieldset>
                    </label>
                    ...
                    <label>
                        ...
                        <input v-model="codeConfirmation" placeholder="Digite o código enviado"
                            aria-describedby="valid-code"
                            :aria-invalid="validCodeConfirmation != undefined ? !validCodeConfirmation : validCodeConfirmation"
                            aria-label="Password" />
                    </label>

                    <ToolButton style="width: 100%;" @click="LoginImpl.passwordResetVerifyCode">Verificar</ToolButton>
                </fieldset>

                <hr />
                <a class="secondary" style="display: block; text-align: center;"
                    @click="LoginImpl.passwordResetClose">Cancelar</a>
            </form>
        </article>
    </div>
...
export default defineComponent({
    data() {
        return {
            ...
            invalidNewPasswordRecovery: LoginImpl.loginData.invalidNewPasswordRecovery,
