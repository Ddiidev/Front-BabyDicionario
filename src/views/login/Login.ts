...
function invalidNewPasswordRecovery(): boolean {
    ...
    if (loginData.newPasswordRecovery.value === undefined || loginData.newPasswordRecovery.value === '') {
...
export async function passwordResetVerifyCode() {
    if (await verifyCode(loginData.email.value ?? '', loginData.newPasswordRecovery.value ?? '', loginData.codeConfirmation.value ?? '')) {
